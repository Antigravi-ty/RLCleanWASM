...
export const VIRTUAL_CARD_IP_REGEX = /\b198\.(18|19)\.\d{1,3}\.\d{1,3}\b/g;

/**
 * Replaces virtual network card IPs (198.18.X.X or 198.19.X.X) generated by TUN/VIF adapters
 * (such as Surge VIF / proxy tap devices) with 127.0.0.1 loopback to guarantee local WebRTC peering.
 * @param {string|object} candidateOrSdp
 * @returns {string|object}
 */
export function sanitizeCandidateIp(candidateOrSdp) {
  if (!candidateOrSdp) return candidateOrSdp;

  if (typeof candidateOrSdp === 'string') {
    return candidateOrSdp.replace(VIRTUAL_CARD_IP_REGEX, '127.0.0.1');
  }

  if (typeof candidateOrSdp === 'object') {
    const copy = { ...candidateOrSdp };
    if (typeof copy.candidate === 'string') {
      copy.candidate = copy.candidate.replace(VIRTUAL_CARD_IP_REGEX, '127.0.0.1');
    }
    if (typeof copy.address === 'string' && /^198\.(18|19)\.\d{1,3}\.\d{1,3}$/.test(copy.address)) {
      copy.address = '127.0.0.1';
    }
    if (typeof copy.ip === 'string' && /^198\.(18|19)\.\d{1,3}\.\d{1,3}$/.test(copy.ip)) {
      copy.ip = '127.0.0.1';
    }
    return copy;
  }

  return candidateOrSdp;
}
...
export class P2PWebRTCChannel {
  ...
  createPeerConnection() {
    ...
    this.pc.onicecandidate = (event) => {
      if (event.candidate) {
        ...
        candJson = sanitizeCandidateIp(candJson);
  ...
  async createOfferToken(options = {}) {
    ...
    if (this.pc) {
      ...
      sdp = sanitizeCandidateIp(this.pc.localDescription.sdp);
    }
  ...
  async acceptOfferAndCreateAnswer(offerToken, options = {}) {
    ...
    if (this.pc) {
      ...
      if (offerData.sdp) {
        const sanitizedOfferSdp = sanitizeCandidateIp(offerData.sdp);
        ...
        if (Array.isArray(offerData.candidates)) {
          for (const cand of offerData.candidates) {
            await this.addRemoteCandidate(sanitizeCandidateIp(cand));
          }
        }
        if (this.pendingRemoteCandidates && this.pendingRemoteCandidates.length > 0) {
          for (const cand of this.pendingRemoteCandidates) {
            try { await this.pc.addIceCandidate(new RTCIceCandidate(sanitizeCandidateIp(cand))); } catch (_) {}
          }
          this.pendingRemoteCandidates = [];
        }
  ...
  async acceptAnswerToken(answerToken) {
    ...
    if (answerData.sdp) {
      const sanitizedAnswerSdp = sanitizeCandidateIp(answerData.sdp);
      await this.pc.setRemoteDescription(new RTCSessionDescription({
        type: 'answer',
  ...
  sendServerState(packet, nowMs = performance.now()) {
    ...
    if (this.extraLatencyMs > 0) {
      const delay = Math.max(0, this.extraLatencyMs + (Math.random() * 2 - 1) * this.jitterMs);
      setTimeout(doTransmitState, delay);
      sent = true;
    } else {
      sent = doTransmitState();
    }
