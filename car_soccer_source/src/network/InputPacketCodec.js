 ...
 * 
 * - Optional Client Timestamp (8 bytes Float64 BE appended at end):
 *   Used for symmetric RTT telemetry echo without modifying core tick payloads.
 * 
 * Total Packet Size: 7 + 10 * 3 = 37 bytes (without timestamp) or 45 bytes (with timestamp).
 */
...
export class InputPacketCodec {
   ...
   * @param {number|null} [clientTimestamp=null] Optional local send timestamp for round-trip RTT telemetry
   * @returns {Uint8Array}
   */
  static encode(carIndex, targetTick, history, clientTimestamp = null) {
    const count = Math.min(255, history.length);
    const hasTimestamp = clientTimestamp !== null && typeof clientTimestamp === 'number' && Number.isFinite(clientTimestamp);
    const totalSize = HEADER_SIZE + count * BYTES_PER_TICK + (hasTimestamp ? 8 : 0);
    ...
    if (hasTimestamp) {
      view.setFloat64(HEADER_SIZE + count * BYTES_PER_TICK, clientTimestamp, false);
    }
   ...
   * @returns {{ carIndex: number, targetTick: number, history: Array<{ tick: number, carIndex: number, controls: object }>, clientTimestamp: number|null }|null}
   */
  static decode(data) {
    if (!data) return null;
    ...
    if (typeof data === 'object' && !(data instanceof Uint8Array) && !(data instanceof ArrayBuffer)) {
      if (data.history) {
        return {
          ...data,
          clientTimestamp: data.clientTimestamp ?? data.timestamp ?? null
        };
      }
    ...
    if (u8.length >= expectedSize + 8) {
      clientTimestamp = view.getFloat64(expectedSize, false);
    }
