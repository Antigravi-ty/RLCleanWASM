...
function resolveContext() {
  const threeContext = vehicleAssemblyThreeContext;
  return {
    ...THREE,
    ...threeContext,
    ...
    multiThemeMaterial: threeContext.multiThemeMaterial,
    getThemeMaterial: threeContext.getThemeMaterial,
    cloneMaterial: threeContext.cloneMaterial,
    createCelShadedToonMaterial: threeContext.createCelShadedToonMaterial,
    setShadowFlags: threeContext.setShadowFlags,
    applyArcadeCelShading: threeContext.applyArcadeCelShading,
    mergeVertices: threeContext.mergeVertices,
    mergeGeometries: threeContext.mergeGeometries
  };
}
...
export function findMeshByMaterialName(root, matName) {
  ...
  root.traverse?.(node => {
    ...
    if (mats.some(mat => mat && mat.name === matName)) result = node;
  });
...
export function detachOctaneWheelHardware(bodyMesh, wheelMeshes, resolveContextFn = resolveContext) {
  ...
  for (let i = 0; i < indexAttr.count; i += 3) {
    const idxA = indexAttr.getX ? indexAttr.getX(i) : indexAttr.array[i];
    const idxB = indexAttr.getX ? indexAttr.getX(i + 1) : indexAttr.array[i + 1];
    const idxC = indexAttr.getX ? indexAttr.getX(i + 2) : indexAttr.array[i + 2];
    unionRoots(idxA, idxB);
    unionRoots(idxB, idxC);
  }
  ...
  for (let i = 0; i < indexAttr.count; i += 3) {
    const firstVertexIdx = indexAttr.getX ? indexAttr.getX(i) : indexAttr.array[i];
    const root = findRoot(firstVertexIdx);
    ...
    for (let triOffset = 0; triOffset < 3; triOffset++) {
      const idx = indexAttr.getX ? indexAttr.getX(i + triOffset) : indexAttr.array[i + triOffset];
  ...
  return wheelWorldPositions.map((wheelPos, wheelIdx) => {
    ...
    for (let vertexSlot = 0; vertexSlot < indices.length; vertexSlot++) {
      const vertIdx = indices[vertexSlot];
      if (tmpVec.fromBufferAttribute) {
      ...
      tmpVec.toArray(positions, vertexSlot * 3);
      ...
      if (normAttr) {
        ...
        tmpNorm.toArray(normals, vertexSlot * 3);
      }
    }
  ...
  root.traverse?.(node => {
    ...
    if (node instanceof Mesh || node.isMesh) {
      ...
      node.material = Array.isArray(node.material)
        ? node.material.map(mat => mapVehicleMaterial(mat, palette, node))
 ...
 */
export function lerpPolyline(points, targetX) {
  if (targetX <= points[0].x) return points[0].clone ? points[0].clone() : { ...points[0] };
  for (let i = 1; i < points.length; i++) {
    if (targetX <= points[i].x) {
      const t = (targetX - points[i - 1].x) / (points[i].x - points[i - 1].x);
      if (points[i - 1].clone) {
...
export function createRealisticCarGimbals(resolveContextFn = resolveContext) {
  ...
  const addTube = (pts, radius, mat, name, parentGroup, closed = false) => {
    ...
    mesh.userData.path = pts.map(point => point.toArray?.() || [point.x, point.y, point.z]);
    return mesh;
  };
  ...
  for (const side of [-1, 1]) {
    ...
    for (const sideSign of [-1, 1]) {
      addTube([
        ...
        new Vector3(sideSign * 2.1, -3.5, side * 14.1),
        new Vector3(sideSign * 3.2, -10.1, side * 12.5),
        new Vector3(sideSign * 3.2, -13.8, side * 11.8),
        new Vector3(sideSign * 3.2, -14.2, side * 11.4),
        new Vector3(sideSign * 3.8, -15.7, side * 7)
  ...
  for (let layer = 0; layer < 2; layer++) {
    for (let profileIndex = 0; profileIndex < spineProfile.length - 1; profileIndex++) {
      for (let s = 0; s < lateralSpans.length - 1; s++) {
        const idxA = layer * numRing + profileIndex * lateralSpans.length + s;
        const idxB = idxA + 1;
        const idxC = idxB + lateralSpans.length;
        const idxD = idxA + lateralSpans.length;
        seatIndices.push(...(layer ? [idxA, idxC, idxB, idxA, idxD, idxC] : [idxA, idxB, idxC, idxA, idxC, idxD]));
      }
    }
  }
...
/**
 * Newton-Raphson 2-DOF gimbal orientation solver & velocity heading alignment.
 * Stabilizes cockpit attitude by aligning inner cradle normal with body-relative world UP.
 */
export function updateRealisticCockpitGimbal(quat, vx, vy, dt, gimbals, resolveContextFn = resolveContext) {
  ...
  const targetUpX = upVec.x, targetUpY = upVec.y, targetUpZ = upVec.z;
  let roll = gimbals.rollAngle;
  let cradle = gimbals.cradleAngle;

  const REGULARIZATION = 0.01;
  const DAMPING = 0.4;
  const MAX_SLEW_RATE = 8;

  // 4 Newton-Raphson optimization iterations
  for (let iter = 0; iter < 4; iter++) {
    const cosRoll = Math.cos(roll);
    const sinRoll = Math.sin(roll);
    const cosCradle = Math.cos(cradle);
    const sinCradleNeg = -Math.sin(cradle);

    const forwardNormalY = cosCradle * cosRoll;
    const forwardNormalZ = cosCradle * sinRoll;

    const errorX = targetUpX - sinCradleNeg;
    const errorY = targetUpY - forwardNormalY;
    const errorZ = targetUpZ - forwardNormalZ;

    const jacobianRollY = -forwardNormalZ;
    const jacobianRollZ = forwardNormalY;

    const jacobianCradleX = -sinRoll * forwardNormalZ - cosRoll * forwardNormalY;
    const jacobianCradleY = cosRoll * sinCradleNeg;
    const jacobianCradleZ = sinRoll * sinCradleNeg;

    const rollMagnitudeSq = jacobianRollY * jacobianRollY + jacobianRollZ * jacobianRollZ;
    const j11 = rollMagnitudeSq + REGULARIZATION + DAMPING * (1 - rollMagnitudeSq);
    const j22 = jacobianCradleX * jacobianCradleX + jacobianCradleY * jacobianCradleY + jacobianCradleZ * jacobianCradleZ + REGULARIZATION;
    const j12 = jacobianRollY * jacobianCradleY + jacobianRollZ * jacobianCradleZ;

    const gradientRoll = jacobianRollY * errorY + jacobianRollZ * errorZ;
    const gradientCradle = jacobianCradleX * errorX + jacobianCradleY * errorY + jacobianCradleZ * errorZ;

    const determinant = j11 * j22 - j12 * j12 || 1e-9;
    roll += (j22 * gradientRoll - j12 * gradientCradle) / determinant;
    cradle += (j11 * gradientCradle - j12 * gradientRoll) / determinant;
  }

  const maxStep = MAX_SLEW_RATE * dt;
  ...
  const cosCradle = Math.cos(gimbals.cradleAngle);
  const sinCradle = Math.sin(gimbals.cradleAngle);
  const forwardDir = new Vector3(cosCradle, Math.cos(gimbals.rollAngle) * sinCradle, Math.sin(gimbals.rollAngle) * sinCradle).applyQuaternion(quat);
...
export function createRealisticCarModel(teamColor, options = {}, resolveContextFn = resolveContext) {
  ...
  const gimbalCenterX = cfg.gimbalCenter.x;
  const centerElevationY = up - H / 2;
  const topElevationY = up + H / 2;
  ...
  const frontX = gimbalCenterX + L * 0.27;
  const rearX = gimbalCenterX - L * 0.3;
  const bayRearX = gimbalCenterX - L * 0.27;
  ...
  const roofY = topElevationY - H * 0.025;
  const beltY = centerElevationY + H * 0.5;
  const noseW = W * 0.16;
  ...
  const getAftProfile = (sign, x) => new Vector3(tailX, centerElevationY + H * 0.54, sign * W * 0.29).lerp(new Vector3(bayRearX, topElevationY, sign * roofW), (x - tailX) / (bayRearX - tailX));
  ...
  for (const side of [-1, 1]) {
    const bottomLongeron = [
      new Vector3(tailX, centerElevationY + H * 0.06, side * W * 0.18),
      new Vector3(minWheelX, centerElevationY, side * getWheelInnerZ(minWheelX)),
      new Vector3(rearX, centerElevationY, side * sideW),
      new Vector3(frontX, centerElevationY, side * sideW),
      new Vector3(maxWheelX, centerElevationY, side * getWheelInnerZ(maxWheelX)),
      new Vector3(noseX, centerElevationY + H * 0.12, side * noseW)
    ];
    const topLongeron = [
      getAftProfile(side, tailX),
      getAftProfile(side, minWheelX),
      new Vector3(bayRearX, topElevationY, side * roofW),
    ...
    const midPillarPt = new Vector3(rearX, centerElevationY + H * 0.7, side * sideW);
    ...
    const pressureLine = bottomLongeron.map(point => point.clone().add(new Vector3(0, tubeRadius * 1.5, -side * tubeRadius * 0.65)));
  ...
  const evalBottomRail = (side, targetX) => lerpPolyline(railMap.get(Math.sign(side)).bottom, targetX);
  const evalTopRail = (side, targetX) => lerpPolyline(railMap.get(Math.sign(side)).top, targetX);
  ...
  const powertrainPos = new Vector3(tailX + L * 0.145, centerElevationY + H * 0.35, 0);
...
// Offroad wheel, suspension and reaction jet helpers imported from VehicleSuspensionSystem.js
