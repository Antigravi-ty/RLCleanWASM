...
export function resolveContext() {
  const threeContext = boostEmitterThreeContext;
  return {
    ...THREE,
    ...threeContext
  };
 ...
 */
export function evalCurveLut(lut, normalizedTime, resolveContextFn = resolveContext) {
  const { MathUtils } = resolveContextFn();
  const clamped = MathUtils.clamp(normalizedTime, 0, 1) * (lut.length - 1);
...
export function createRealisticBoostConeGeometry(radius, length, radialSegs = 28, heightSegs = 18, resolveContextFn = resolveContext) {
  ...
  for (let heightStep = 0; heightStep <= heightSegs; heightStep++) {
    const fraction = heightStep / heightSegs;
    const profile = Math.max(0.012, (0.12 + 0.88 * (1 - Math.exp(-fraction * 13))) * Math.pow(1 - fraction, 0.62));
    const currentRadius = radius * profile;
    const posX = -length * fraction;

    for (let radialStep = 0; radialStep <= radialSegs; radialStep++) {
      const uCoord = radialStep / radialSegs;
      const angle = uCoord * Math.PI * 2;
      positions.push(posX, Math.cos(angle) * currentRadius, Math.sin(angle) * currentRadius);
      uvs.push(uCoord, 1 - fraction);
    }
  }
  ...
  for (let heightStep = 0; heightStep < heightSegs; heightStep++) {
    for (let radialStep = 0; radialStep < radialSegs; radialStep++) {
      const quadA = heightStep * stride + radialStep;
      const quadB = quadA + stride;
      indices.push(quadA, quadB, quadA + 1, quadB, quadB + 1, quadA + 1);
    }
  }
...
export function createArcadeBoostFlameConeGeometry(radius, resolveContextFn = resolveContext) {
  ...
  const HEIGHT_SEGS = 12;
  const RADIAL_SEGS = 12;
  const stride = RADIAL_SEGS + 1;

  for (let heightStep = 0; heightStep <= HEIGHT_SEGS; heightStep++) {
    const fraction = heightStep / HEIGHT_SEGS;
    const currentRadius = radius * (0.22 + 0.78 * Math.sin(Math.PI * Math.pow(fraction, 0.68))) * (1 - fraction);
    for (let radialStep = 0; radialStep <= RADIAL_SEGS; radialStep++) {
      const uCoord = radialStep / RADIAL_SEGS;
      const angle = uCoord * Math.PI * 2;
      positions.push(-50 * fraction, Math.cos(angle) * currentRadius, Math.sin(angle) * currentRadius);
      uvs.push(uCoord, 1 - fraction);
    }
  }

  for (let heightStep = 0; heightStep < HEIGHT_SEGS; heightStep++) {
    for (let radialIndex = 0; radialIndex < RADIAL_SEGS; radialIndex++) {
      const quadA = heightStep * stride + radialIndex;
      const quadB = quadA + stride;
      indices.push(quadA, quadB, quadA + 1, quadB, quadB + 1, quadA + 1);
    }
  }
