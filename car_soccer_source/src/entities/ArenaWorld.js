...
export function resolveContext() {
  const threeContext = arenaWorldThreeContext;
  return {
    ...THREE,
    ...threeContext,
    GLTFLoader: threeContext.GLTFLoader,
    OBJLoader: threeContext.OBJLoader,
    VehicleBoostEmitter: threeContext.VehicleBoostEmitter,
    multiThemeMaterial: threeContext.multiThemeMaterial || ((m1, m2) => m1),
    getThemeMaterial: threeContext.getThemeMaterial || ((m, mode) => m),
    cloneMaterial: threeContext.cloneMaterial || ((m) => ({ ...m })),
    markMatrixDirty: threeContext.markMatrixDirty || (() => {}),
...
export class ArenaWorld {
  ...
  addCar(teamIndex, visual = this.carVisual) {
    ...
    if (isHitbox) {
      ...
      const frontAxleOffset = cfg.forward + cfg.length * 0.35, rearAxleOffset = cfg.forward - cfg.length * 0.35, sideOffset = (cfg.width * 0.5) + 5;
      wheelSpecs = [[frontAxleOffset, sideOffset, 12.5], [frontAxleOffset, -sideOffset, 12.5], [rearAxleOffset, sideOffset, 15], [rearAxleOffset, -sideOffset, 15]];
    } else {
  ...
  update(prevState, currState, alpha, delta = 0, throttle = 0, controls, opponentControls, activeVisuals = true) {
    ...
    for (let carIndex = 0; carIndex < this.cars.length; carIndex++) {
      ...
      const carThrottle = carIndex === 0 ? actualThrottle : (actualOpponentControls?.throttle ?? 0);
  ...
  updateBoostVisuals(currState, throttle, delta, opponentThrottle = 0, activeVisuals = true) {
    ...
    for (let carIndex = 0; carIndex < this.cars.length; carIndex++) {
      ...
      const isBoosting = activeVisuals && carView.isBoosting;
      const isAudibleActive = carIndex === 0;
      const currentThrottle = isAudibleActive ? throttle : opponentThrottle;
