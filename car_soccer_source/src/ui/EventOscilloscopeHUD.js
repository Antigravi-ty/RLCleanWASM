...
import { applySurfaceTaxonomyColoring } from '../audio/AudioArchitecture.js';
...
export const PARAM_PRESETS = Object.freeze({
  ...
  normalRelVel: { label: 'Normal Rel Vel', defaultMin: 0, defaultMax: 2000, sliderMin: 0, sliderMax: 4000, step: 20 },
  speed: { label: 'Speed', defaultMin: 0, defaultMax: 2500, sliderMin: 0, sliderMax: 4000, step: 25 },
...
export class EventOscilloscopeHUD {
  ...
  constructor(container = (typeof document !== 'undefined' ? document.body : null), options = {}) {
    ...
    this.physicsEngine = options.physicsEngine || null;
    ...
    this.threshold = 0;
    this.cooldownMs = 40;
    this.arenaWorld = options.arenaWorld || null;
    this.floorNz = 0.80;
    this.netNz = 0.05;
    this.dirDot = 0.00;
    this.netAngle = 60.0;
    this.taxonomyColoringEnabled = false; // milliseconds
  ...
  _initDom() {
    ...
    root.setAttribute('style', 'background-color: rgba(11, 19, 36, 0.88); backdrop-filter: blur(11px); -webkit-backdrop-filter: blur(11px);');
    root.hidden = true;
      ...
      <div class="scope-controls" data-el="controls">
        <div class="scope-grid-row">
          <div class="scope-control-item">
            <label for="scope-event-filter">Event Source</label>
            <select id="scope-event-filter" class="scope-select" data-el="eventFilterSelect">
              <option value="all">All Events</option>
              <option value="ball_ground">Ball Ground Impact</option>
              <option value="ball_world">Ball World / Wall</option>
              <option value="car_ball">Car-Ball Hit</option>
              <option value="ball_goalpost">Goalpost / Bar</option>
              <option value="car_car">Car-Car Collision</option>
            </select>
          </div>
          <div class="scope-control-item">
        ...
        <div class="scope-grid-row">
          <div class="scope-control-item">
            <label>Impact Threshold <span class="val-badge" data-el="thresholdVal">0</span></label>
            <div class="scope-dual-box">
          ...
          <div class="scope-control-item">
            <label>Audio Cooldown <span class="val-badge" data-el="cooldownVal">40ms</span></label>
            <div class="scope-dual-box">
              <input type="range" class="scope-slider" min="0" max="500" step="1" value="40" data-el="cooldownSlider" />
              <input type="number" class="scope-input-num" min="0" max="500" step="1" value="40" data-el="cooldownInput" />
            </div>
          </div>
        </div>

        <div class="scope-grid-row" style="margin-top: 4px; padding-top: 6px; border-top: 1px solid rgba(255,255,255,0.1);">
          <div class="scope-control-item" style="grid-column: span 2;">
            <label style="display: flex; align-items: center; gap: 6px; cursor: pointer; color: #a7f3d0; font-weight: 600;">
              <input type="checkbox" data-el="cbTaxonomyColoring" style="accent-color: #10b981; cursor: pointer;" />
              <span>Debug Surface Taxonomy Mesh Coloring</span>
            </label>
          </div>
        </div>

        <div class="scope-grid-row" data-el="taxonomyControls">
          <div class="scope-control-item">
            <label>Floor Normal Nz <span class="val-badge" data-el="floorNzVal">0.80</span></label>
            <div class="scope-dual-box">
              <input type="range" class="scope-slider" min="0.5" max="0.99" step="0.01" value="0.80" data-el="floorNzSlider" />
              <input type="number" class="scope-input-num" min="0.5" max="0.99" step="0.01" value="0.80" data-el="floorNzInput" />
            </div>
          </div>
          <div class="scope-control-item">
            <label>Net/Ceiling Nz <span class="val-badge" data-el="netNzVal">0.05</span></label>
            <div class="scope-dual-box">
              <input type="range" class="scope-slider" min="-0.2" max="0.3" step="0.01" value="0.05" data-el="netNzSlider" />
              <input type="number" class="scope-input-num" min="-0.2" max="0.3" step="0.01" value="0.05" data-el="netNzInput" />
            </div>
          </div>
        </div>
        ...
        <div class="scope-grid-row">
          <div class="scope-control-item">
            <label>Direction Dot Threshold <span class="val-badge" data-el="dirDotVal">0.00</span></label>
            <div class="scope-dual-box">
              <input type="range" class="scope-slider" min="0.0" max="0.5" step="0.01" value="0.00" data-el="dirDotSlider" />
              <input type="number" class="scope-input-num" min="0.0" max="0.5" step="0.01" value="0.00" data-el="dirDotInput" />
            </div>
          </div>
          <div class="scope-control-item">
            <label>Net Dual-Sound Angle <span class="val-badge" data-el="netAngleVal">60°</span></label>
            <div class="scope-dual-box">
              <input type="range" class="scope-slider" min="30" max="85" step="1" value="60" data-el="netAngleSlider" />
              <input type="number" class="scope-input-num" min="30" max="85" step="1" value="60" data-el="netAngleInput" />
            </div>
          </div>
        </div>

        <div class="scope-grid-row" style="margin-top: 6px; display: flex; align-items: center; justify-content: space-between;">
          <div style="font-size: 11px; color: rgba(255, 255, 255, 0.65);">
            <span>Bridge: </span>
            <span data-el="bridgeStatus" style="color: #70d6ff; font-weight: 500;">Ready</span>
          </div>
          <button class="scope-btn scope-btn--update" data-el="btnUpdateBridge" style="background: rgba(0, 150, 255, 0.25); border: 1px solid rgba(0, 180, 255, 0.6); color: #70d6ff; font-weight: 600; padding: 4px 12px; border-radius: 4px; cursor: pointer; transition: all 0.2s;" title="Update C++ Bridge & Audio Thresholds">Update C++ Bridge</button>
        </div>
      </div>
  ...
  _bindEvents() {
    ...
    setupDualInput(cooldownSlider, cooldownInput, cooldownVal, (v) => {
      this.cooldownMs = Math.min(500, Math.max(0, Math.round(v)));
    ...
    // Taxonomy Coloring checkbox
    query('[data-el="cbTaxonomyColoring"]')?.addEventListener('change', (e) => {
      this.taxonomyColoringEnabled = Boolean(e.target.checked);
      if (this.arenaWorld) {
        applySurfaceTaxonomyColoring(this.arenaWorld, this.taxonomyColoringEnabled, this.floorNz, this.netNz);
      }
    });

    setupDualInput(query('[data-el="floorNzSlider"]'), query('[data-el="floorNzInput"]'), query('[data-el="floorNzVal"]'), (v) => {
      this.floorNz = Number(v);
      if (this.taxonomyColoringEnabled && this.arenaWorld) {
        applySurfaceTaxonomyColoring(this.arenaWorld, true, this.floorNz, this.netNz);
      }
    });

    setupDualInput(query('[data-el="netNzSlider"]'), query('[data-el="netNzInput"]'), query('[data-el="netNzVal"]'), (v) => {
      this.netNz = Number(v);
      if (this.taxonomyColoringEnabled && this.arenaWorld) {
        applySurfaceTaxonomyColoring(this.arenaWorld, true, this.floorNz, this.netNz);
      }
    ...
    setupDualInput(query('[data-el="dirDotSlider"]'), query('[data-el="dirDotInput"]'), query('[data-el="dirDotVal"]'), (v) => {
      this.dirDot = Number(v);
    });

    setupDualInput(query('[data-el="netAngleSlider"]'), query('[data-el="netAngleInput"]'), query('[data-el="netAngleVal"]'), (v) => {
      this.netAngle = Math.round(Number(v));
      const badge = query('[data-el="netAngleVal"]');
      if (badge) badge.textContent = `${this.netAngle}°`;
    });
  ...
  updateBridgeThresholds() {
    const cooldownTicks = Math.max(1, Math.round(this.cooldownMs / (1000 / 120)));
    let bridgeUpdated = false;
    if (this.physicsEngine && typeof this.physicsEngine.setImpactThresholds === "function") {
      try {
        this.physicsEngine.setImpactThresholds(this.threshold, this.threshold, this.threshold, cooldownTicks);
        if (typeof this.physicsEngine.setSurfaceTaxonomyThresholds === "function") {
          this.physicsEngine.setSurfaceTaxonomyThresholds(this.floorNz, this.netNz);
        }
        if (typeof this.physicsEngine.setDirectionDotThreshold === "function") {
          this.physicsEngine.setDirectionDotThreshold(this.dirDot);
        }
        bridgeUpdated = true;
      } catch (e) {
        console.warn("[EventOscilloscopeHUD] Failed to update C++ bridge thresholds:", e);
      }
    }

    try {
      setCarBallHitImpulseThreshold(this.threshold);
      setGoalpostHitThreshold(this.threshold);
      setCarBallHitCooldownMs(this.cooldownMs);
      setGoalpostHitCooldownMs(this.cooldownMs);
    } catch (e) {}

    const btnUpdate = this.root?.querySelector("[data-el=\"btnUpdateBridge\"]");
    const btnSync = this.root?.querySelector("[data-el=\"btnSyncAudio\"]");
    const status = this.root?.querySelector("[data-el=\"bridgeStatus\"]");

    if (btnUpdate) {
      const origText = btnUpdate.textContent;
      btnUpdate.textContent = "✓ Updated!";
      btnUpdate.style.borderColor = "#4caf50";
      btnUpdate.style.color = "#81c784";
      setTimeout(() => {
        if (btnUpdate) {
          btnUpdate.textContent = origText;
          btnUpdate.style.borderColor = "rgba(0, 180, 255, 0.6)";
          btnUpdate.style.color = "#70d6ff";
        }
      }, 1500);
    }

    if (btnSync) {
      const origSync = btnSync.textContent;
      btnSync.textContent = "Synced!";
      setTimeout(() => { if (btnSync) btnSync.textContent = origSync; }, 1200);
    }

    if (status) {
      status.textContent = `Synced (${this.threshold} / ${this.cooldownMs}ms${bridgeUpdated ? " -> C++" : ""})`;
    }
  }

  syncThresholdToAudio() {
    this.updateBridgeThresholds();
  }
  ...
  addEvent(event) {
    ...
    // Fallback: If impulse is 0 (e.g. ball ground/world/goalpost impacts), use normalRelVel or speed
    if ((!clonedEvent.impulse || clonedEvent.impulse === 0) && (clonedEvent.normalRelVel || clonedEvent.speed || clonedEvent.relSpeed)) {
      clonedEvent.impulse = Math.abs(clonedEvent.normalRelVel || clonedEvent.speed || clonedEvent.relSpeed || 0);
    }
  ...
  update() {
    ...
    for (let i = 0; i < this.history.length; i++) {
      ...
      const ev = item.event;
      if (this.eventFilter === 'ball_ground') {
        const isFloor = ev.surfaceTag === 1 || ev.surfaceTag === SURFACE_SUBTYPES.FLOOR || (ev.z <= 25.0 && ev.normalZ > 0.7);
        if (ev.type !== INCIDENT_EVENT_TYPES.BALL_WORLD_HIT || !isFloor) continue;
      } else if (this.eventFilter === 'ball_world') {
        if (ev.type !== INCIDENT_EVENT_TYPES.BALL_WORLD_HIT) continue;
      } else if (this.eventFilter === 'car_ball') {
        if (ev.type !== INCIDENT_EVENT_TYPES.CAR_BALL_HIT) continue;
      } else if (this.eventFilter === 'ball_goalpost') {
        if (ev.type !== INCIDENT_EVENT_TYPES.BALL_GOALPOST_HIT && !ev.isCrossbar) continue;
      } else if (this.eventFilter === 'car_car') {
        if (ev.type !== INCIDENT_EVENT_TYPES.CAR_CAR_BUMP && ev.type !== INCIDENT_EVENT_TYPES.CAR_CAR_DEMO && ev.type !== INCIDENT_EVENT_TYPES.CAR_CAR_COLLISION) continue;
      }
