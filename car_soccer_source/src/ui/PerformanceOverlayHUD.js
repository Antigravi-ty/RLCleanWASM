...
export class PerformanceProfiler {
  ...
  frameEnd(ticks, dropped = 0, stalled = false) {
    ...
    for (let phaseIndex = 0; phaseIndex < PROFILER_PHASES.length; phaseIndex += 1) {
      this.phaseMs[phaseIndex][writeIdx] = this.pending[phaseIndex];
    }
  ...
  snapshot(historyBins = 240, historySeconds = 10) {
    ...
    const frameStats = this.statsOf(this.frameMs, sampleCount);
    const cpuStats = this.statsOf(this.cpuMs, sampleCount);
    ...
    for (let phaseIndex = 0; phaseIndex < PROFILER_PHASES.length; phaseIndex += 1) {
      let phaseSum = 0;
      for (let i = 0; i < sampleCount; i += 1) {
        phaseSum += this.phaseMs[phaseIndex][this.at(i)];
      }
      const phaseAvg = phaseSum / sampleCount;
      totalPhasesAvg += phaseAvg;
      phases.push({ phase: PROFILER_PHASES[phaseIndex], avgMs: phaseAvg, share: 0 });
    }
...
export class PerformanceOverlayHUD {
  ...
  constructor(container, profiler, renderer, settings, onDetailsChange = () => {}, options = {}) {
    ...
    this.timer = 0;
    this.compact = typeof window !== 'undefined' ? window.matchMedia('(max-width: 860px), (max-height: 500px), (pointer: coarse)') : null;
    this.summary = null;
    this.details = null;
    this.expanded = false;
          ...
          <div class="status__section" data-section="fps" hidden>
            <div class="status__hero">
              <span class="status__pair" title="Current frames per second, screen refresh rate, and render scale.">
                ...
                <span class="status__scale" data-el="statScaleSimple" style="color:rgba(255,255,255,0.75);margin-left:8px;font-weight:500;font-size:0.9em;display:inline-block;vertical-align:bottom;line-height:1.3;">—</span>
              </span>
            </div>
          </div>
  ...
  applyDisclosure() {
    const isCompact = this.compact?.matches ?? false;
  ...
  draw() {
    ...
    const phyRateNum = Number(this.options?.getPhysicsRate?.() ?? 120);
    const phyRateStr = `Phy:${phyRateNum.toFixed(1)}`;

    let latencyInfo = this.options?.getNetworkLatencyInfo?.();
    let latencyStr = null;
    if (latencyInfo && latencyInfo.baseLatency !== undefined && latencyInfo.baseLatency !== null && latencyInfo.baseLatency >= 0) {
      const base = Math.round(latencyInfo.baseLatency);
      const extra = Math.round(latencyInfo.extraLatency || 0);
      latencyStr = `${base}ms(+${extra})`;
    } else {
      const rttMs = this.options?.getNetworkRtt?.();
      if (rttMs !== undefined && rttMs !== null && rttMs >= 0) {
        latencyStr = `${Math.round(rttMs * 0.5)}ms(+0)`;
      }
    }
    ...
    if (s.fps) {
      ...
      let scaleText = `${currentScale}% · ${phyRateStr}`;
      if (latencyStr) {
        scaleText += `<br><span style="color:#d2a8ff;font-weight:600;font-size:0.9em;letter-spacing:0.3px;">Latency ${latencyStr}</span>`;
      }
      this.set('statScaleSimple', scaleText);
    }

    const phyBadge = `<span style="color:#58a6ff;margin-left:6px;font-weight:500;font-size:0.85em;">${phyRateStr}</span>`;
    const latencyBadge = latencyStr ? `<br><span style="color:#d2a8ff;font-weight:600;font-size:0.85em;">Latency ${latencyStr}</span>` : '';

    if (s.fps && !s.frame) {
      this.set('summaryFps', `${snap.frame.fps.toFixed(0)} <span style="color:rgba(255,255,255,0.45);font-weight:400;font-size:0.85em;">/ ${Math.round(snap.refreshHz || 60)}</span> <span style="color:rgba(255,255,255,0.75);margin-left:6px;font-weight:500;font-size:0.85em;">${currentScale}%</span> ${phyBadge}${latencyBadge}`);
      const msSpan = this.root.querySelector('[data-el="summaryMs"]');
      if (msSpan?.parentElement) msSpan.parentElement.hidden = true;
    } else if (s.frame) {
      this.set('summaryFps', `${snap.frame.fps.toFixed(0)} <span style="color:rgba(255,255,255,0.75);margin-left:4px;font-size:0.85em;">${currentScale}%</span> ${phyBadge}${latencyBadge}`);
    ...
    if (!(this.compact?.matches && !this.expanded)) {
      ...
      if (s.phases && !s.phasesCollapsed) {
        this.rows('statPhases', [
          ...snap.phases.filter(phaseStat => phaseStat.phase !== 'other' || phaseStat.avgMs >= 0.02).map(phaseStat => [phaseStat.phase, `${phaseStat.avgMs.toFixed(2)} ms`]),
