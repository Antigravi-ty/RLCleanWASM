...
export class SpeedometerHUD {
  constructor(container = document.body) {
    ...
    // Physics rate indicator label
    this.phyRateLabel = document.createElement('div');
    this.phyRateLabel.id = 'hud-speedometer-phy-rate';
    this.phyRateLabel.style.cssText = `
      position: absolute;
      left: 0;
      bottom: 12px;
      font-size: 9px;
      ...
      pointer-events: none;
    `;
    this.phyRateLabel.textContent = 'Phy:120.0';

    this.root.appendChild(this.notch);
    this.root.appendChild(this.notchLabel);
    this.root.appendChild(this.phyRateLabel);
  ...
  update(rawSpeed) {
    ...
    this.bar.style.width = `${fillRatio}%`;
  ...
  /**
   * Updates the physics rate label on the speedometer
   * @param {number} rate
   */
  updatePhysicsRate(rate) {
    if (this.phyRateLabel) {
      this.phyRateLabel.textContent = `Phy:${Number(rate).toFixed(1)}`;
    }
  }
