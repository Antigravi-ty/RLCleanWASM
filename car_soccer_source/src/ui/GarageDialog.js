...
export const garageSettingsStore = createLocalStorageStore(
  ...
  (target, stored) => {
    ...
    target.carVisual = target.vehiclePreset; // backward compatibility
  }
);
...
export class GarageTurntable {
  ...
  getThree() {
    return this.three || garageThreeContext || THREE;
  }
  ...
  dispose() {
    ...
    for (const [, turntable] of this.turntables) {
...
export class GarageDialog {
        ...
        <div id="car-overlay" class="car-overlay" hidden aria-hidden="true">
          <section class="car-dialog" role="dialog" aria-modal="true" aria-labelledby="car-dialog-title">
            ...
            <footer class="car-dialog__foot">
              <span>${renderIcon("check", 16)} Switching cars equips vehicle instantly.</span>
  ...
  syncCards() {
    const currentPreset = this.display.vehiclePreset || this.display.carVisual;
    const rideLabel = typeof document !== 'undefined' ? document.querySelector(".car-tab__ride") : null;
    if (rideLabel) {
      const match = CAR_VISUAL_OPTIONS.find((opt) => opt.id === currentPreset);
      rideLabel.textContent = match?.label ?? "";
    }
    for (const card of this.cards()) {
      const isEquipped = card.dataset.carChoice === currentPreset;
  ...
  choose(visualId) {
    const currentPreset = this.display.vehiclePreset || this.display.carVisual;
    if (currentPreset === visualId) {
      this.hide();
      return;
    }
    this.display.vehiclePreset = visualId;
    ...
    if (typeof this.onSelectCar === 'function') {
      try {
        this.onSelectCar(visualId);
      } catch (err) {
        console.error("[GarageDialog] Failed to hot-swap vehicle:", err);
      }
    }
    if (typeof window !== 'undefined') {
      window.setTimeout(() => {
        if (this.overlay) {
          this.overlay.classList.remove("is-committing");
        }
        this.hide();
      }, 200);
    }
