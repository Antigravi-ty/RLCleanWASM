...
export class MatchDialog {
  constructor(container, options) {
      ...
      <div id="match-overlay" class="match-overlay" hidden inert aria-hidden="true">
        <section id="match-dialog" class="match-panel" role="dialog" aria-modal="true"
          ...
          <div class="match-panel__body">
            <div class="match-panel__warning" data-match="worker-warning" role="note" style="margin-bottom: 14px; padding: 10px 14px; background: rgba(234, 179, 8, 0.14); border: 1px solid rgba(234, 179, 8, 0.4); border-radius: 8px; color: #fef08a; font-size: 13px; line-height: 1.4; align-items: center; gap: 10px; display: none;" hidden>
              <span style="font-size: 16px; flex-shrink: 0;" aria-hidden="true">⚠️</span>
              <span data-match="worker-warning-text">Necessary file is missing. AI opponent worker is not available and will not work as intended (running in fallback heuristic mode).</span>
            </div>
            ...
            <fieldset class="match-difficulty" aria-describedby="match-difficulty-help">
              ...
              <div class="match-difficulty__options">
                ${BOT_POLICIES.map(policy => `
                  <button class="match-difficulty__option" type="button" data-bot-id="${policy.id}"
                          aria-pressed="false" aria-label="${policy.rank}: ${policy.name}">
                    <span class="match-difficulty__rank">${policy.rank}</span>
                    <span class="match-difficulty__name">${policy.name}</span>
                  </button>
                `).join("")}
              </div>
    ...
    this.workerWarning = this.element("worker-warning");
    ...
    this.overlay.addEventListener("click", (overlayEvent) => {
      if (overlayEvent.target === this.overlay) this.hide();
    });
    ...
    document.addEventListener("focusin", (focusEvent) => {
      if (this.openState && !this.dialog.contains(focusEvent.target)) {
        this.focusInitial();
      }
    });
  ...
  isWorkerMissing() {
    return Boolean(this.options.isAiWorkerMissing?.() || this.workerMissing || this.view?.workerMissing);
  }
  ...
  update(state) {
    ...
    if (typeof state.workerMissing === "boolean") {
      this.workerMissing = state.workerMissing;
    }
    this.view = { ...state };
    this.render();
  }
  ...
  render() {
    ...
    const isMissingWorker = this.isWorkerMissing();
    if (this.workerWarning) {
      this.workerWarning.hidden = !isMissingWorker;
      this.workerWarning.style.display = isMissingWorker ? "flex" : "none";
    }
  ...
  destroy() {
    if (typeof cancelAnimationFrame !== "undefined" && this.padPoll) {
    ...
    if (this.tab && this.tab.parentElement) {
