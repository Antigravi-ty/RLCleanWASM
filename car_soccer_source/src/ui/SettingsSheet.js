 ...
 * - Training rules (Free play goal restart, unlimited boost, hitbox wireframe)
...
import {
  ...
  setEngineVolume,
  setBoostVolume,
  setVehicleSelfVolume,
  setJumpVolume,
  setDoubleJumpVolume,
  setDodgeVolume,
  setSupersonicVolume,
  setCarCarCollisionVolume,
  setBallCollisionVolume,
  setCarBallHitVolume,
  setBallWorldHitVolume,
  setGoalpostHitVolume,
  setBallRollingVolume,
  setBallFlyingVolume,
  setGoalScoredVolume,
  setFlipResetVolume,
  setBoostCollectVolume,
  setCarCarSoundMode,
  setCarBallHitCooldownMs,
  setGoalpostHitCooldownMs,
  setCarBallHitImpulseThreshold,
  setGoalpostHitThreshold,
  setContinueAudioOnLostFocus,
  setLogAllEvents,
  setLogCarBallHitEvents,
...
export const SETTINGS_TABS = Object.freeze(["camera", "controls", "graphics", "audio", "training", "trajectory", "diagnostics"]);
...
export class SettingsSheet {
  constructor(container, target, trainingTarget, bindings, onOpenChange, onTrainingChange, onBindingsChange, onCaptureChange) {
    ...
    this.pollPadCapture = () => {
      ...
      if (pad) {
        for (let btnIndex = 0; btnIndex < pad.buttons.length; btnIndex++) {
          const pressed = pad.buttons[btnIndex]?.pressed ?? false;
          if (pressed && !this.capturePrevButtons[btnIndex]) {
            this.applyCapture({
              kind: "padButton",
              index: btnIndex
            });
          ...
          this.capturePrevButtons[btnIndex] = pressed;
        }
    ...
    container.insertAdjacentHTML("beforeend", `
      <div class="hud-tools">
        <button id="oscilloscope-button" class="hud-tool" type="button"
                aria-label="Toggle physics oscilloscope" title="Physics Oscilloscope (O)">
          <span data-el="oscilloscopeIcon">${renderIcon("chart")}</span>
        </button>
        ...
        <section class="sheet" role="dialog" aria-modal="true" aria-labelledby="settings-title">
          ...
          <nav class="sheet-tabs" aria-label="Settings categories" role="tablist">
            ...
            ${[
              ...
              ["trajectory", "Predictor", "trajectory"],
              ["diagnostics", "Status", "chart"]
            ].map(([category, label, iconName]) => `
              <button id="settings-tab-${category}" class="sheet-tab${category === "camera" ? " is-active" : ""}"
                      type="button" role="tab" aria-selected="${category === "camera"}"
                      aria-controls="settings-panel-${category}" data-settings-tab="${category}">
                ${renderIcon(iconName, 24)}
                <span class="sheet-tab__name">${label}</span>
              </button>
            `).join("")}
          ...
          <div class="sheet-body">
            <section id="settings-panel-camera" class="sheet-panel" role="tabpanel"
              ...
              ${CAMERA_SETTING_GROUPS.map(group => `
                <section class="zone">
                  <header class="zone__head">
                    <span class="zone__letter" aria-hidden="true">${group.letter}</span>
                    <h2 class="zone__label">${group.label}</h2>
                    <p class="zone__note">${group.note}</p>
                  </header>
                  <div class="zone__rows">
                    ${group.ranges.map(rangeKey => renderCameraRangeInput(CAMERA_RANGE_SETTINGS_MAP.get(rangeKey))).join("")}
                    ${group.checks.map(checkSetting => renderCheckboxDim(checkSetting.key, checkSetting.label, checkSetting.note, "camera")).join("")}
                  </div>
                </section>
              `).join("")}
            </section>
              ...
              <section class="zone">
                ...
                <div class="theme-options" role="group" aria-label="Game theme" aria-describedby="theme-description">
                  ${[
                    ...
                    <button type="button" class="theme-option theme-option--${category}" data-theme-option="${category}"
                            aria-label="${label} theme" aria-pressed="${getTheme() === category}" aria-describedby="theme-description-${category}">
                      ...
                      <span class="theme-option__body">
                        <span class="theme-option__heading"><span class="theme-option__name">${label}</span>${category === DEFAULT_THEME ? '<span class="theme-option__default">Default</span>' : ""}</span>
                        <span class="theme-option__description" id="theme-description-${category}">${description}</span>
                        <span class="theme-option__selection">${renderIcon("check", 17)}<span data-theme-selection>${getTheme() === category ? "Selected" : "Choose theme"}</span></span>
                      </span>
                    </button>
                  `).join("")}
                </div>
            ...
            <section id="settings-panel-audio" class="sheet-panel" role="tabpanel"
              ...
              <p class="panel-lede">
                Independently adjust volume levels for each audio element (0% - 500%). Supports slider drag or direct numeric input (press Enter to confirm).
              </p>

              <!-- Section A: Master & Core Bus -->
              <section class="zone">
                <header class="zone__head">
                  <span class="zone__letter" aria-hidden="true">A</span>
                  <h2 class="zone__label">Master Output & Background</h2>
                  <p class="zone__note">Master output volume and background playback options</p>
                </header>
                <div class="zone__rows">
                  <div class="dim dim--flag" data-dim-row="continueAudioOnLostFocus">
                    <label class="dim__label" for="audio-continue-on-lost-focus">Play Audio in Background</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control dim__control--flag">
                      <input id="audio-continue-on-lost-focus" class="tick" type="checkbox" data-audio-setting="continueAudioOnLostFocus" />
                      <span class="tick__mark" aria-hidden="true"></span>
                      <span class="dim__note">When disabled, audio is muted on window blur and buffer is flushed to prevent audio pop</span>
                    </span>
                  </div>
                  <div class="dim" data-dim-row="masterVolume">
                    <label class="dim__label" for="audio-master-volume">Master Volume</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-master-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="masterVolume" />
                      <input id="audio-master-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="masterVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="masterVolume" for="audio-master-volume"></output>
                  </div>
                  <div class="dim" data-dim-row="engineVolume">
                    <label class="dim__label" for="audio-engine-volume">Engine Volume</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-engine-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="engineVolume" />
                      <input id="audio-engine-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="engineVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="engineVolume" for="audio-engine-volume"></output>
                  </div>
                  <div class="dim" data-dim-row="boostVolume">
                    <label class="dim__label" for="audio-boost-volume">Boost Volume</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-boost-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="boostVolume" />
                      <input id="audio-boost-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="boostVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="boostVolume" for="audio-boost-volume"></output>
                  </div>
                </div>
              </section>

              <!-- Section B: Layer 1 Vehicle Self Audio -->
              <section class="zone">
                <header class="zone__head">
                  <span class="zone__letter" aria-hidden="true">B</span>
                  <h2 class="zone__label">Layer 1: Vehicle Actions</h2>
                  <p class="zone__note">Jump, dodge/flip, supersonic, and flip reset sound effects</p>
                </header>
                <div class="zone__rows">
                  <div class="dim" data-dim-row="vehicleSelfVolume">
                    <label class="dim__label" for="audio-vehicle-self-volume">Vehicle Actions Master</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-vehicle-self-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="vehicleSelfVolume" />
                      <input id="audio-vehicle-self-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="vehicleSelfVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="vehicleSelfVolume" for="audio-vehicle-self-volume"></output>
                  </div>
                  <div class="dim" data-dim-row="jumpVolume">
                    <label class="dim__label" for="audio-jump-volume">Single Jump</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-jump-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="jumpVolume" />
                      <input id="audio-jump-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="jumpVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="jumpVolume" for="audio-jump-volume"></output>
                  </div>
                  <div class="dim" data-dim-row="doubleJumpVolume">
                    <label class="dim__label" for="audio-double-jump-volume">Double Jump</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-double-jump-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="doubleJumpVolume" />
                      <input id="audio-double-jump-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="doubleJumpVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="doubleJumpVolume" for="audio-double-jump-volume"></output>
                  </div>
                  <div class="dim" data-dim-row="dodgeVolume">
                    <label class="dim__label" for="audio-dodge-volume">Dodge / Flip</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-dodge-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="dodgeVolume" />
                      <input id="audio-dodge-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="dodgeVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="dodgeVolume" for="audio-dodge-volume"></output>
                  </div>
                  <div class="dim" data-dim-row="supersonicVolume">
                    <label class="dim__label" for="audio-supersonic-volume">Supersonic Enter</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-supersonic-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="supersonicVolume" />
                      <input id="audio-supersonic-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="supersonicVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="supersonicVolume" for="audio-supersonic-volume"></output>
                  </div>
                  <div class="dim" data-dim-row="flipResetVolume">
                    <label class="dim__label" for="audio-flip-reset-volume">Flip Reset Sound Effect</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-flip-reset-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="flipResetVolume" />
                      <input id="audio-flip-reset-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="flipResetVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="flipResetVolume" for="audio-flip-reset-volume"></output>
                  </div>
                </div>
              </section>

              <!-- Section C: Layer 2 Vehicle Interaction & Boost -->
              <section class="zone">
                <header class="zone__head">
                  <span class="zone__letter" aria-hidden="true">C</span>
                  <h2 class="zone__label">Layer 2: Vehicle Interaction & SFX</h2>
                  <p class="zone__note">Vehicle bumps, demolitions, and boost pad collection</p>
                </header>
                <div class="zone__rows">
                  <div class="dim" data-dim-row="carCarCollisionVolume">
                    <label class="dim__label" for="audio-car-car-volume">Car Collision & Demo</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-car-car-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="carCarCollisionVolume" />
                      <input id="audio-car-car-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="carCarCollisionVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="carCarCollisionVolume" for="audio-car-car-volume"></output>
                  </div>
                  <div class="dim" data-dim-row="carCarSoundMode">
                    <label class="dim__label" for="audio-car-car-sound-mode">Car Collision Sound Mode</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control">
                      <select id="audio-car-car-sound-mode" class="pick" data-audio-setting="carCarSoundMode">
                        <option value="sfx_collision">SFX Car Collision (Classic)</option>
                        <option value="vehicle_detail">Vehicle Detail (6 Dynamic Random Variations)</option>
                      </select>
                    </span>
                  </div>
                  <div class="dim" data-dim-row="boostCollectVolume">
                    <label class="dim__label" for="audio-boost-collect-volume">Boost Pad Collect</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-boost-collect-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="boostCollectVolume" />
                      <input id="audio-boost-collect-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="boostCollectVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="boostCollectVolume" for="audio-boost-collect-volume"></output>
                  </div>
                </div>
              </section>

              <!-- Section D: Layer 3 Ball Physics & Ambient Loops -->
              <section class="zone">
                <header class="zone__head">
                  <span class="zone__letter" aria-hidden="true">D</span>
                  <h2 class="zone__label">Layer 3: Ball SFX & Continuous Loops</h2>
                  <p class="zone__note">Ball hit, wall impact, goalpost collision, surface roll, and aerial whoosh</p>
                </header>
                <div class="zone__rows">
                  <div class="dim" data-dim-row="ballCollisionVolume">
                    <label class="dim__label" for="audio-ball-collision-volume">Ball Collision Master</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-ball-collision-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="ballCollisionVolume" />
                      <input id="audio-ball-collision-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="ballCollisionVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="ballCollisionVolume" for="audio-ball-collision-volume"></output>
                  </div>
                  <div class="dim" data-dim-row="carBallHitVolume">
                    <label class="dim__label" for="audio-car-ball-hit-volume">Car-Ball Hit</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-car-ball-hit-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="carBallHitVolume" />
                      <input id="audio-car-ball-hit-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="carBallHitVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="carBallHitVolume" for="audio-car-ball-hit-volume"></output>
                  </div>
                  <div class="dim" data-dim-row="ballWorldHitVolume">
                    <label class="dim__label" for="audio-ball-world-hit-volume">Ball-World / Wall Hit</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-ball-world-hit-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="ballWorldHitVolume" />
                      <input id="audio-ball-world-hit-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="ballWorldHitVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="ballWorldHitVolume" for="audio-ball-world-hit-volume"></output>
                  </div>
                  <div class="dim" data-dim-row="goalpostHitVolume">
                    <label class="dim__label" for="audio-goalpost-hit-volume">Goalpost / Crossbar Hit</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-goalpost-hit-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="goalpostHitVolume" />
                      <input id="audio-goalpost-hit-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="goalpostHitVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="goalpostHitVolume" for="audio-goalpost-hit-volume"></output>
                  </div>
                  <div class="dim" data-dim-row="ballRollingVolume">
                    <label class="dim__label" for="audio-ball-rolling-volume">Ball Rolling Loop</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-ball-rolling-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="ballRollingVolume" />
                      <input id="audio-ball-rolling-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="ballRollingVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="ballRollingVolume" for="audio-ball-rolling-volume"></output>
                  </div>
                  <div class="dim" data-dim-row="ballFlyingVolume">
                    <label class="dim__label" for="audio-ball-flying-volume">Ball Flying Loop</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-ball-flying-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="ballFlyingVolume" />
                      <input id="audio-ball-flying-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="ballFlyingVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="ballFlyingVolume" for="audio-ball-flying-volume"></output>
                  </div>
                  <div class="dim" data-dim-row="goalScoredVolume">
                    <label class="dim__label" for="audio-goal-scored-volume">Goal Explosion Poof</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-goal-scored-volume" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="goalScoredVolume" />
                      <input id="audio-goal-scored-volume-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="goalScoredVolume"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="goalScoredVolume" for="audio-goal-scored-volume"></output>
                  </div>
                </div>
              </section>

              <!-- Section E: Physics Incident Thresholds & Cooldowns -->
              <section class="zone">
                <header class="zone__head">
                  <span class="zone__letter" aria-hidden="true">E</span>
                  <h2 class="zone__label">Thresholds & Audio Shielding</h2>
                  <p class="zone__note">Debounce cooldown intervals and real collision impulse thresholds</p>
                </header>
                <div class="zone__rows">
                  <div class="dim" data-dim-row="carBallHitCooldownMs">
                    <label class="dim__label" for="audio-carball-cooldown">Car-Ball Audio Debounce (0-100ms)</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-carball-cooldown" class="dim__line" type="range"
                             min="0" max="100" step="1" data-audio-setting="carBallHitCooldownMs" />
                      <input id="audio-carball-cooldown-input" class="dim__num-input" type="number"
                             min="0" max="100" step="1" data-audio-num-input="carBallHitCooldownMs"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="carBallHitCooldownMs" for="audio-carball-cooldown"></output>
                  </div>
                  <div class="dim" data-dim-row="goalpostHitCooldownMs">
                    <label class="dim__label" for="audio-goalpost-cooldown">Goalpost Audio Debounce (0-500ms)</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-goalpost-cooldown" class="dim__line" type="range"
                             min="0" max="500" step="5" data-audio-setting="goalpostHitCooldownMs" />
                      <input id="audio-goalpost-cooldown-input" class="dim__num-input" type="number"
                             min="0" max="500" step="1" data-audio-num-input="goalpostHitCooldownMs"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="goalpostHitCooldownMs" for="audio-goalpost-cooldown"></output>
                  </div>
                  <div class="dim" data-dim-row="carBallHitImpulseThreshold">
                    <label class="dim__label" for="audio-carball-hit-impulse-threshold">Car-Ball Impulse Threshold</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-carball-hit-impulse-threshold" class="dim__line" type="range"
                             min="0" max="1000" step="5" data-audio-setting="carBallHitImpulseThreshold" />
                      <input id="audio-carball-hit-impulse-threshold-input" class="dim__num-input" type="number"
                             min="0" max="1000" step="5" data-audio-num-input="carBallHitImpulseThreshold"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="carBallHitImpulseThreshold" for="audio-carball-hit-impulse-threshold"></output>
                  </div>
                  <div class="dim" data-dim-row="goalpostHitThreshold">
                    <label class="dim__label" for="audio-goalpost-hit-threshold">Goalpost Hit Threshold (Live Collision)</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; align-items: center; gap: 8px;">
                      <input id="audio-goalpost-hit-threshold" class="dim__line" type="range"
                             min="0" max="1000" step="5" data-audio-setting="goalpostHitThreshold" />
                      <input id="audio-goalpost-hit-threshold-input" class="dim__num-input" type="number"
                             min="0" max="1000" step="5" data-audio-num-input="goalpostHitThreshold"
                             style="width: 56px; padding: 2px 4px; font-size: 11px; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; text-align: right;" />
                    </span>
                    <output class="figure" data-audio-value-for="goalpostHitThreshold" for="audio-goalpost-hit-threshold"></output>
                  </div>
                  <div class="dim dim--flag" data-dim-row="logAllEvents">
                    <label class="dim__label" for="audio-log-all-events">Console Log All Events</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control dim__control--flag">
                      <input id="audio-log-all-events" class="tick" type="checkbox" data-audio-setting="logAllEvents" />
                      <span class="tick__mark" aria-hidden="true"></span>
                      <span class="dim__note">Log all physics events and signals in developer console in real time</span>
                    </span>
                  </div>
                  <div class="dim dim--flag" data-dim-row="logCarBallHitEvents">
                    <label class="dim__label" for="audio-log-carball-hit-events">Log Car-Ball Hit Events Only</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control dim__control--flag">
                      <input id="audio-log-carball-hit-events" class="tick" type="checkbox" data-audio-setting="logCarBallHitEvents" />
                      <span class="tick__mark" aria-hidden="true"></span>
                      <span class="dim__note">Log car-ball hit event structs in console</span>
                    </span>
                  </div>

                  <!-- Console Log Event Filters Panel -->
                  <div class="event-log-filters-panel" style="margin: 10px 0 16px 0; padding: 12px; background: rgba(0, 0, 0, 0.35); border: 1px solid rgba(255, 255, 255, 0.12); border-radius: 6px;">
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px;">
                      <span style="font-size: 12px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; color: #cbd5e1;">Console Event Stream Filters</span>
                      <div style="display: flex; gap: 8px;">
                        <button type="button" id="btn-select-all-event-logs" class="act-btn" style="padding: 3px 10px; font-size: 11px; background: rgba(59, 130, 246, 0.25); border: 1px solid rgba(59, 130, 246, 0.6); color: #93c5fd; border-radius: 4px; cursor: pointer;">Select All</button>
                        <button type="button" id="btn-deselect-all-event-logs" class="act-btn" style="padding: 3px 10px; font-size: 11px; background: rgba(239, 68, 68, 0.25); border: 1px solid rgba(239, 68, 68, 0.6); color: #fca5a5; border-radius: 4px; cursor: pointer;">Deselect All</button>
                      </div>
                    </div>
                    <div id="event-log-filter-items-container" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 6px; max-height: 250px; overflow-y: auto; padding-right: 4px;">
                      ${CONSOLE_LOG_EVENT_CATALOG.map(item => `
                        <label class="event-log-item" style="display: flex; align-items: center; gap: 8px; padding: 6px 10px; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.1); border-radius: 5px; cursor: pointer; user-select: none; transition: border-color 0.15s ease, background 0.15s ease, opacity 0.15s ease;">
                          <input class="event-log-filter-checkbox" type="checkbox" data-event-hex="${item.hex}" style="width: 15px; height: 15px; margin: 0; accent-color: #38bdf8; cursor: pointer; flex-shrink: 0;" />
                          <span style="font-family: monospace; font-size: 11px; padding: 2px 5px; background: rgba(0,0,0,0.5); border-radius: 3px; color: #38bdf8; font-weight: 600;">${item.hex}</span>
                          <span style="font-size: 11px; color: #e2e8f0; font-weight: 500;">${item.id}</span>
                          <span style="font-size: 10px; color: #94a3b8; margin-left: auto;">${item.name}</span>
                        </label>
                      `).join('')}
                    </div>
                  </div>

                  <!-- Audio Preset JSON Management -->
                  <div class="dim dim--flag" style="margin-top: 10px;">
                    <label class="dim__label">Configuration Persistence (audioConfig.json)</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display: flex; gap: 8px;">
                      <button id="btn-audio-export-json" type="button" class="field-btn" style="padding: 4px 12px; font-size: 11px;" title="Export configuration to audioConfig.json">Export JSON</button>
                      <button id="btn-audio-import-json" type="button" class="field-btn" style="padding: 4px 12px; font-size: 11px;" title="Import external audioConfig.json configuration">Import JSON</button>
                      <input id="input-audio-import-file" type="file" accept=".json" style="display: none;" />
                    </span>
                  </div>
                </div>
              </section>
            ...
            <section id="settings-panel-diagnostics" class="sheet-panel" role="tabpanel"
              ...
              <section class="zone">
                ...
                <div class="zone__rows">
                  ${STATUS_SETTING_ITEMS.map(settingItem => renderCheckboxDim(settingItem.key, settingItem.label, settingItem.note, "status")).join("")}
                </div>
              </section>
              <section class="zone">
                <header class="zone__head">
                  <h2 class="zone__label">Physics Oscilloscope</h2>
                  <p class="zone__note">Real-time parameter oscilloscope for tuning event thresholds.</p>
                </header>
                <div class="status-master">
                  <button class="act" type="button" data-scope-toggle style="width: 100%;">Toggle Event Oscilloscope</button>
                </div>
              </section>
              <section class="zone">
                <header class="zone__head">
                  <h2 class="zone__label">Dual-Arena Determinism Harness</h2>
                  <p class="zone__note">Live side-by-side reconciliation testing with continuous rollback & bit-exact delta verification. (Shift+O)</p>
                </header>
                <div class="status-master">
                  <button class="act" type="button" data-determinism-toggle style="width: 100%;">Toggle Dual-Arena HUD (Shift+O)</button>
                </div>
              </section>
              <section class="zone">
                <header class="zone__head">
                  <h2 class="zone__label">Multiplayer Client Prediction & Reconciliation</h2>
                  <p class="zone__note">Network latency & jitter stress testing with predictive rollback and visual error smoothing. (Shift+P)</p>
                </header>
                <div class="status-master">
                  <button class="act" type="button" data-net-predict-toggle style="width: 100%;">Toggle Network HUD (Shift+P)</button>
                </div>
              </section>
            </section>
            ...
            <section id="settings-panel-trajectory" class="sheet-panel" role="tabpanel"
                     aria-labelledby="settings-tab-trajectory" data-settings-panel="trajectory" tabindex="0" hidden>
              <section class="zone">
                <header class="zone__head">
                  <span class="zone__letter" aria-hidden="true">P</span>
                  <h2 class="zone__label">Ball Trajectory Predictor</h2>
                  <p class="zone__note">Client-side RocketSim WASM trajectory simulation triggered on car-ball impact incidents.</p>
                </header>
                <div class="zone__rows">
                  <div class="dim dim--flag">
                    <label class="dim__label" for="sheet-traj-enabled">Enable Trajectory Predictor</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control dim__control--flag">
                      <input id="sheet-traj-enabled" class="tick" type="checkbox" data-traj-sheet="enabled" />
                      <span class="tick__mark" aria-hidden="true"></span>
                      <span class="dim__note">Draw ball flight and bounce prediction line in local arena.</span>
                    </span>
                  </div>

                  <div class="dim dim--flag">
                    <label class="dim__label" for="sheet-traj-dynamic-mode">Ball Pushes Line (Dynamic Mode)</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control dim__control--flag">
                      <input id="sheet-traj-dynamic-mode" class="tick" type="checkbox" data-traj-sheet="dynamicMode" />
                      <span class="tick__mark" aria-hidden="true"></span>
                      <span class="dim__note">Unchecked: fixed in air (ball follows line); Checked: flows forward (ball pushes line).</span>
                    </span>
                  </div>
                </div>
              </section>
              ...
              <section class="zone">
                <header class="zone__head">
                  <span class="zone__letter" aria-hidden="true">T</span>
                  <h2 class="zone__label">Prediction Timing & Dash Intervals</h2>
                  <p class="zone__note">Configure distance duration and dashed/solid duty cycle in 120Hz ticks.</p>
                </header>
                <div class="zone__rows">
                  <div class="dim">
                    <label class="dim__label" for="sheet-traj-dist">Prediction Distance</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control">
                      <input id="sheet-traj-dist" class="range" type="range" min="60" max="600" step="1" data-traj-sheet="predictionTicks" />
                    </span>
                    <output class="figure" id="sheet-traj-dist-val" for="sheet-traj-dist">600 ticks (5.00s)</output>
                  </div>

                  <div class="dim">
                    <label class="dim__label" for="sheet-traj-solid">Solid Line Duration</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control">
                      <input id="sheet-traj-solid" class="range" type="range" min="5" max="240" step="1" data-traj-sheet="solidTicks" />
                    </span>
                    <output class="figure" id="sheet-traj-solid-val" for="sheet-traj-solid">30 ticks (250ms)</output>
                  </div>

                  <div class="dim">
                    <label class="dim__label" for="sheet-traj-trans">Transparent Gap Duration</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control">
                      <input id="sheet-traj-trans" class="range" type="range" min="5" max="240" step="1" data-traj-sheet="transparentTicks" />
                    </span>
                    <output class="figure" id="sheet-traj-trans-val" for="sheet-traj-trans">10 ticks (83ms)</output>
                  </div>
                </div>
              </section>
              ...
              <section class="zone">
                <header class="zone__head">
                  <span class="zone__letter" aria-hidden="true">V</span>
                  <h2 class="zone__label">Visual Styling</h2>
                  <p class="zone__note">Adjust trajectory line thickness, opacity, and color.</p>
                </header>
                <div class="zone__rows">
                  <div class="dim">
                    <label class="dim__label" for="sheet-traj-thickness">Line Thickness (Ball %)</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control">
                      <input id="sheet-traj-thickness" class="range" type="range" min="1" max="75" step="1" data-traj-sheet="thickness" />
                    </span>
                    <output class="figure" id="sheet-traj-thickness-val" for="sheet-traj-thickness">15%</output>
                  </div>

                  <div class="dim">
                    <label class="dim__label" for="sheet-traj-opacity">Opacity</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control">
                      <input id="sheet-traj-opacity" class="range" type="range" min="0.05" max="1.00" step="0.05" data-traj-sheet="opacity" />
                    </span>
                    <output class="figure" id="sheet-traj-opacity-val" for="sheet-traj-opacity">85%</output>
                  </div>

                  <div class="dim">
                    <label class="dim__label" for="sheet-traj-color">Line Color</label>
                    <span class="dim__leader" aria-hidden="true"></span>
                    <span class="dim__control" style="display:flex;align-items:center;gap:10px;">
                      <input id="sheet-traj-color" type="color" data-traj-sheet="color" style="appearance:none;-webkit-appearance:none;border:none;width:28px;height:28px;cursor:pointer;background:none;border-radius:6px;" />
                      <button type="button" class="chip" data-sheet-swatch="#000000" style="background:#000000;color:#fff;font-weight:700;border:1px solid rgba(255,255,255,0.3);">Black</button>
                      <button type="button" class="chip" data-sheet-swatch="#00f0ff" style="background:#00f0ff;color:#000;font-weight:700;">Cyan</button>
                      <button type="button" class="chip" data-sheet-swatch="#39ff14" style="background:#39ff14;color:#000;font-weight:700;">Lime</button>
                      <button type="button" class="chip" data-sheet-swatch="#fbbf24" style="background:#fbbf24;color:#000;font-weight:700;">Amber</button>
                      <button type="button" class="chip" data-sheet-swatch="#f97316" style="background:#f97316;color:#fff;font-weight:700;">Orange</button>
                    </span>
                    <output class="figure" id="sheet-traj-color-val" for="sheet-traj-color">#00F0FF</output>
                  </div>
                </div>
              </section>
              ...
              <section class="zone">
                <header class="zone__head">
                  <h2 class="zone__label">Quick Actions</h2>
                  <p class="zone__note">Launch standalone draggable menu or trigger immediate live test prediction.</p>
                </header>
                <div style="display:flex;gap:12px;padding:12px 16px;">
                  <button class="act" type="button" data-sheet-traj-open-modal style="flex:1;">Open Standalone Menu Window</button>
                  <button class="act act--primary" type="button" data-sheet-traj-test style="flex:1;">Trigger Test Prediction</button>
                </div>
              </section>
            </section>
          </div>
    ...
    // Toolbar buttons
    container.querySelector("#oscilloscope-button")?.addEventListener("click", () => {
      if (typeof window !== "undefined") {
        window.dispatchEvent(new CustomEvent("car-soccer:toggle-oscilloscope"));
      }
    });
    this.overlay?.querySelector("[data-scope-toggle]")?.addEventListener("click", () => {
      if (typeof window !== "undefined") {
        window.dispatchEvent(new CustomEvent("car-soccer:toggle-oscilloscope"));
      }
    });
    this.overlay?.querySelector("[data-determinism-toggle]")?.addEventListener("click", () => {
      if (typeof window !== "undefined") {
        window.dispatchEvent(new CustomEvent("car-soccer:toggle-determinism"));
      }
    });
    this.overlay?.querySelector("[data-net-predict-toggle]")?.addEventListener("click", () => {
      if (typeof window !== "undefined") {
        window.dispatchEvent(new CustomEvent("car-soccer:toggle-network-hud"));
      }
    });
    ...
    // Wire console event log filter buttons
    const btnSelectAllLogs = this.overlay?.querySelector('#btn-select-all-event-logs');
    if (btnSelectAllLogs) {
    ...
    const btnDeselectAllLogs = this.overlay?.querySelector('#btn-deselect-all-event-logs');
    if (btnDeselectAllLogs) {
      btnDeselectAllLogs.addEventListener('click', () => {
        deselectAllEventLogs();
        this.syncAudioControls();
      });
    }

    this.overlay?.querySelectorAll('.event-log-filter-checkbox').forEach(input => {
      input.addEventListener('change', () => {
        const hex = input.dataset.eventHex;
        if (hex) {
          setEventLogFilterItem(hex, input.checked);
          this.syncAudioControls();
        }
      });
    });

    // Wire audio JSON export / import
    const btnExport = this.overlay?.querySelector('#btn-audio-export-json');
    if (btnExport) {
      btnExport.addEventListener('click', () => {
        ...
        this.setStatus('audioConfig.json exported!');
      });
    }

    const btnImport = this.overlay?.querySelector('#btn-audio-import-json');
    const fileImport = this.overlay?.querySelector('#input-audio-import-file');
    if (btnImport && fileImport) {
      ...
      fileImport.addEventListener('change', (e) => {
        ...
        reader.onload = (evt) => {
          ...
          if (content && importAudioSettingsJson(content)) {
            this.syncAudioControls();
            this.setStatus('Audio config imported successfully!');
          } else {
            this.setStatus('Failed to import audio config.');
          }
        };
    ...
    this.overlay?.querySelector("#controller-input")?.addEventListener("change", (e) => {
      ...
      const pad = getConnectedGamepads().find(gamepad => gamepad?.connected && String(gamepad.index) === val);
      if (val === "auto" || pad) {
    ...
    if (typeof window !== "undefined") {
      ...
      window.addEventListener(AUDIO_SETTINGS_CHANGED_EVENT, () => this.syncAudioControls());
    }

    this.syncCameraControls();
    this.syncAudioControls();
    this.syncTrainingControls();
    this.syncTrajectoryControls();
    this.setupTrajectoryListeners();
  ...
  attachTrajectoryPredictor(predictor, predictorHUD, physics) {
    this.trajectoryPredictor = predictor;
    this.trajectoryHUD = predictorHUD;
    this.physics = physics;
    this.syncTrajectoryControls();
  }

  syncTrajectoryControls() {
    const settings = this.trajectoryPredictor ? this.trajectoryPredictor.settings : loadSavedTrajectorySettings();
    const panel = this.overlay?.querySelector('#settings-panel-trajectory');
    if (!panel) return;
    ...
    setChecked('[data-traj-sheet="enabled"]', settings.enabled);
    setChecked('[data-traj-sheet="dynamicMode"]', settings.dynamicMode);
    setVal('[data-traj-sheet="predictionTicks"]', settings.predictionTicks);
    setVal('[data-traj-sheet="solidTicks"]', settings.solidTicks);
    setVal('[data-traj-sheet="transparentTicks"]', settings.transparentTicks);
    setVal('[data-traj-sheet="thickness"]', settings.thickness);
    setVal('[data-traj-sheet="opacity"]', settings.opacity);
    setVal('[data-traj-sheet="color"]', settings.color);

    setText('#sheet-traj-dist-val', `${settings.predictionTicks} ticks (${(settings.predictionTicks / 120).toFixed(2)}s)`);
    setText('#sheet-traj-solid-val', `${settings.solidTicks} ticks (${(settings.solidTicks / 120 * 1000).toFixed(0)}ms)`);
    setText('#sheet-traj-trans-val', `${settings.transparentTicks} ticks (${(settings.transparentTicks / 120 * 1000).toFixed(0)}ms)`);
    setText('#sheet-traj-thickness-val', `${settings.thickness}%`);
    setText('#sheet-traj-opacity-val', `${Math.round(settings.opacity * 100)}%`);
    setText('#sheet-traj-color-val', settings.color.toUpperCase());
  }

  setupTrajectoryListeners() {
    const panel = this.overlay?.querySelector('#settings-panel-trajectory');
    if (!panel) return;

    const onUpdate = (key, value) => {
      const current = this.trajectoryPredictor ? this.trajectoryPredictor.settings : loadSavedTrajectorySettings();
      current[key] = value;
      saveTrajectorySettings(current);
      if (this.trajectoryPredictor) {
        this.trajectoryPredictor.updateSettings(current);
      }
      if (this.trajectoryHUD) {
        this.trajectoryHUD.settings = { ...current };
        this.trajectoryHUD.syncFormValues();
      }
      this.syncTrajectoryControls();
    };

    panel.querySelector('[data-traj-sheet="enabled"]')?.addEventListener('change', (e) => {
      onUpdate('enabled', e.target.checked);
    });
    panel.querySelector('[data-traj-sheet="dynamicMode"]')?.addEventListener('change', (e) => {
      onUpdate('dynamicMode', e.target.checked);
    });
    panel.querySelector('[data-traj-sheet="predictionTicks"]')?.addEventListener('input', (e) => {
      onUpdate('predictionTicks', Number(e.target.value));
    });
    panel.querySelector('[data-traj-sheet="solidTicks"]')?.addEventListener('input', (e) => {
      onUpdate('solidTicks', Number(e.target.value));
    });
    panel.querySelector('[data-traj-sheet="transparentTicks"]')?.addEventListener('input', (e) => {
      onUpdate('transparentTicks', Number(e.target.value));
    });
    panel.querySelector('[data-traj-sheet="thickness"]')?.addEventListener('input', (e) => {
      onUpdate('thickness', Number(e.target.value));
    });
    panel.querySelector('[data-traj-sheet="opacity"]')?.addEventListener('input', (e) => {
      onUpdate('opacity', Number(e.target.value));
    });
    panel.querySelector('[data-traj-sheet="color"]')?.addEventListener('input', (e) => {
      onUpdate('color', e.target.value);
    });

    panel.querySelectorAll('[data-sheet-swatch]').forEach(btn => {
      btn.addEventListener('click', () => {
        const color = btn.getAttribute('data-sheet-swatch');
        if (color) onUpdate('color', color);
      });
    });

    panel.querySelector('[data-sheet-traj-open-modal]')?.addEventListener('click', () => {
      this.hide();
      this.trajectoryHUD?.show();
    });

    panel.querySelector('[data-sheet-traj-test]')?.addEventListener('click', () => {
      if (this.trajectoryPredictor && this.physics) {
        this.trajectoryPredictor.predictFromCurrentState(this.physics);
      } else if (this.trajectoryHUD) {
        this.trajectoryHUD.triggerTestPrediction();
      }
    });
  }
  ...
  show() {
    ...
    this.stopRendering = srEl ? srEl.checked : true;
  ...
  updateAudioSetting(input) {
    ...
    if (input.tagName === "SELECT" || key === "carCarSoundMode") {
      setCarCarSoundMode(input.value);
      return;
    }
    if (input.type === 'checkbox') {
      if (key === 'continueAudioOnLostFocus') {
        setContinueAudioOnLostFocus(input.checked);
      } else if (key === 'logAllEvents') {
        setLogAllEvents(input.checked);
        if (input.checked) {
          selectAllEventLogs();
        } else {
          deselectAllEventLogs();
        ...
        this.syncAudioControls();
      } else if (key === 'logCarBallHitEvents') {
        setLogCarBallHitEvents(input.checked);
      }
      return;
    }
    const val = Number(input.value);
    ...
    else if (key === "vehicleSelfVolume") setVehicleSelfVolume(val / 100);
    else if (key === "carCarCollisionVolume") setCarCarCollisionVolume(val / 100);
    else if (key === "ballCollisionVolume") setBallCollisionVolume(val / 100);
    else if (key === "carBallHitCooldownMs") setCarBallHitCooldownMs(val);
    else if (key === "carBallHitImpulseThreshold") setCarBallHitImpulseThreshold(val);
  ...
  syncAudioControls() {
    ...
    this.overlay?.querySelectorAll("[data-audio-setting]").forEach(input => {
      const key = input.dataset.audioSetting;
      if (input.tagName === "SELECT" || key === "carCarSoundMode") {
        input.value = s.carCarSoundMode || "sfx_collision";
        return;
      }
      if (input.type === 'checkbox') {
        if (key === 'continueAudioOnLostFocus') {
          input.checked = Boolean(s.continueAudioOnLostFocus);
        } else if (key === 'logAllEvents') {
          input.checked = Boolean(s.logAllEvents);
        } else if (key === 'logCarBallHitEvents') {
          input.checked = Boolean(s.logCarBallHitEvents);
        }
        return;
      }
      let v;
      if (key === "engineVolume") v = s.engineVolume ?? 0.8;
      else if (key === "boostVolume") v = s.boostVolume ?? 0.8;
      else if (key === "vehicleSelfVolume") v = s.vehicleSelfVolume ?? 1.0;
      else if (key === "carCarCollisionVolume") v = s.carCarCollisionVolume ?? 1.0;
      else if (key === "ballCollisionVolume") v = s.ballCollisionVolume ?? 1.0;
      else if (key === "carBallHitCooldownMs") v = s.carBallHitCooldownMs ?? 40;
      else if (key === "carBallHitImpulseThreshold") v = s.carBallHitImpulseThreshold ?? 0;
      else v = s.masterVolume ?? 1.0;

      if (key === "carBallHitImpulseThreshold" || key === "carBallHitCooldownMs") {
    ...
    const filter = s.logEventFilter;
    this.overlay?.querySelectorAll('.event-log-filter-checkbox').forEach(input => {
      const hex = input.dataset.eventHex;
      if (hex) {
        const isChecked = filter ? filter[hex] !== false : true;
        input.checked = isChecked;
        const label = input.closest('.event-log-item');
        if (label) {
          if (isChecked) {
            label.style.borderColor = 'rgba(56, 189, 248, 0.65)';
            label.style.background = 'rgba(56, 189, 248, 0.12)';
            label.style.opacity = '1.0';
            label.style.boxShadow = '0 0 6px rgba(56, 189, 248, 0.18)';
          } else {
  ...
  updateAudioRangePresentation(input) {
    ...
    const numInput = this.overlay?.querySelector(`[data-audio-num-input="${key}"]`);
    if (numInput && document.activeElement !== numInput) {
      numInput.value = String(val);
    }
    if (key.endsWith('ImpulseThreshold') || key.endsWith('Threshold')) {
      input.setAttribute('aria-valuetext', `${val}`);
      if (output) output.value = `${val}`;
    } else if (key.endsWith('CooldownMs')) {
      input.setAttribute('aria-valuetext', `${val} ms`);
      if (output) output.value = `${val} ms`;
    } else {
  ...
  refreshPadStatus() {
    ...
    for (const gamepad of getConnectedGamepads()) {
      if (gamepad?.connected) {
        options.push(new Option(`${gamepad.id || "Controller"} · ${gamepad.index + 1}`, String(gamepad.index)));
      }
    }
  ...
  applyCapture(binding) {
    const captureState = this.capture;
    if (!captureState) return;
    const res = assignBinding(this.bindings, captureState.device, captureState.action, captureState.slot, binding);
    this.cancelCapture();
    if (!res.changed) {
  ...
  restoreDefaults() {
    ...
    if (this.activeTab === "trajectory") {
      this.trajectoryHUD?.resetToDefaults();
      this.syncTrajectoryControls();
      this.setStatus("Trajectory predictor settings restored to defaults.");
      return;
    }
