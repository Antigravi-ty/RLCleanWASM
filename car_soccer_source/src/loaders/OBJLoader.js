...
export function resolveOBJContext() {
  const threeContext = objThreeContext;
  return {
    ...THREE,
    ...threeContext,
    SRGBColorSpace: threeContext.SRGBColorSpace || THREE.SRGBColorSpace || SRGBColorSpace
  };
}
  ...
  parse(text) {
    ...
    for (let i = 0, len = lines.length; i < len; i++) {
      ...
      if (firstChar === 'v') {
        ...
        const faceVertices = [];
        for (let faceIdx = 0; faceIdx < rawFaces.length; faceIdx++) {
          const item = rawFaces[faceIdx];
          if (item.length > 0) faceVertices.push(item.split('/'));
        }
        const v0 = faceVertices[0];
        for (let triIdx = 1; triIdx < faceVertices.length - 1; triIdx++) {
          const v1 = faceVertices[triIdx];
          const v2 = faceVertices[triIdx + 1];
        ...
        if (line.indexOf('/') === -1) {
          ...
          for (let partIdx = 0; partIdx < lineParts.length; partIdx++) {
            const spl = lineParts[partIdx].split('/');
    ...
    if (hasMeaningfulObjects) {
      for (let objIdx = 0, numObjects = parser.objects.length; objIdx < numObjects; objIdx++) {
        const objData = parser.objects[objIdx];
        ...
        const materialsList = [];
        for (let matIdx = 0, numMaterials = matConfigs.length; matIdx < numMaterials; matIdx++) {
          const matConfig = matConfigs[matIdx];
        ...
        if (materialsList.length > 1) {
          for (let matIdx = 0, numMaterials = matConfigs.length; matIdx < numMaterials; matIdx++) {
            const matConfig = matConfigs[matIdx];
            geometry.addGroup(matConfig.groupStart, matConfig.groupCount, matIdx);
          }
