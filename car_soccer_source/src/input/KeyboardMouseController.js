...
export function isEventWithinUI(target) {
  return (
    ...
    target.closest(
      ".hud-tools, .sheet-overlay, .car-tab, .car-overlay, .match-tab, .match-overlay, .match-hud, .match-result, .status, .touch-controls, .event-oscilloscope, .online-overlay, .online-modal, .online-tab-btn, .net-hud, #online-button, .trajectory-overlay, .trajectory-panel, #trajectory-button"
    ) !== null
  );
}
...
export class KeyboardMouseController {
  constructor(bindings) {
    ...
    this._onKeyDown = (e) => {
      ...
      const isTextEntry = (target instanceof HTMLInputElement && (target.type === "text" || target.type === "number" || target.type === "search" || target.type === "password")) || target instanceof HTMLTextAreaElement;
