...
export class TouchControls {
  constructor(container) {
    ...
    if (typeof document !== "undefined" && container) {
      ...
      this._onCoarsePointerChange = () => this.syncVisibility();
      this.coarsePointer.addEventListener?.("change", this._onCoarsePointerChange);
      ...
      window.addEventListener("pointerdown", this._onWindowPointerDown, { capture: true, passive: true });

      this._onTouchSettingsChanged = (e) => {
        this.clearState();
      ...
      window.addEventListener(TOUCH_SETTINGS_CHANGED_EVENT, this._onTouchSettingsChanged);

      this._onWindowResize = () => this.scheduleJoystickRebuild();
      this._onWindowBlur = () => this.clearState();
      this._onVisibilityChange = () => {
        if (typeof document !== "undefined" && document.hidden) this.clearState();
      };
  ...
  destroy() {
    ...
    if (typeof window !== "undefined") {
      ...
      if (this._onWindowPointerDown) window.removeEventListener("pointerdown", this._onWindowPointerDown, { capture: true, passive: true });
      if (this._onTouchSettingsChanged) window.removeEventListener(TOUCH_SETTINGS_CHANGED_EVENT, this._onTouchSettingsChanged);
      ...
      if (this.coarsePointer?.removeEventListener && this._onCoarsePointerChange) {
        this.coarsePointer.removeEventListener("change", this._onCoarsePointerChange);
      }
    }
