 ...
 * 豁免黑盒专区：固化不可修改的外部黑盒模型 (Immutable External POV Camera Microkernel)
  ...
  async init(options = {}) {
    ...
    if (!wasmBytes) {
      ...
      if (options.locateFile) {
        ...
        wasmPath = '/custom/assets/physics/camera.wasm';
      } else {
        // Resolve relative to repository custom assets path
        wasmPath = new URL('../../../../custom/assets/physics/camera.wasm', import.meta.url).pathname;
      }
